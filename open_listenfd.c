...
// the socket descriptor the specified port.//bind() associates the socket descriptor created
// with socket() to the port we want the server to listen on.
// Once the descriptor is bound, the listen() call//will prepare the socket so that we can call accept() on it
